...
fn named_entry<'a>(
    ...
    selection: &Selection,
    id_column: &str,
) -> crate::Result<Option<&'a CatalogEntry>> {
    let Some(name) = name else {
        return Ok(None);
    };
    if name.is_empty() || name.len() > 256 {
        return Err(PvError::Query("Invalid persistent index name".into()));
    }
    let entry = db.named_retrieval_entry(name)?;
    if entry.definition.table != selection.table || entry.definition.id_column != id_column {
        return Err(PvError::Query(
            "Named retrieval index table/ID definition mismatch".into(),
        ));
    }
    Ok(Some(entry))
}
    ...
    for column in required_columns {
        if !columns.iter().any(|actual| actual == column) {
            return Err(PvError::Query(format!(
                "Missing retrieval column `{column}`"
            )));
        }
    }
    let id = columns
        .iter()
        .position(|column| column == &entry.definition.id_column)
        .ok_or_else(|| PvError::Query("Missing ID column".into()))?;
    if rows.len() > crate::persistent::MAX_RETRIEVAL_DOCUMENTS {
        return Err(PvError::Query("Retrieval filter-ID limit exceeded".into()));
    }
    let mut ids = BTreeSet::new();
    for row in rows {
        let Some(Value::Int(value)) = row.get(id) else {
        ...
        if !entry.contains(*value) {
            return Err(crate::persistent::corrupt(
                "current SELECT and index IDs disagree",
            ));
        }
    }
    Ok(ids)
...
fn text_hits(
    ...
    limit: usize,
    name: Option<&str>,
) -> crate::Result<Vec<crate::search::SearchHit>> {
    ...
    if let Some(entry) = entry {
        ...
        {
            return Err(PvError::Query(
                "Named full-text index column/kind definition mismatch".into(),
            ));
        }
        if selection.accelerate {
            ...
            let index = entry
                ...
                .text
                .as_ref()
                .ok_or_else(|| crate::persistent::corrupt("missing admitted text data"))?;
            return index
                .search_filtered(query, limit, &ids)
                .map_err(|error| PvError::Query(error.to_string()));
        }
    }
...
fn vector_hits(
    ...
    limit: usize,
    name: Option<&str>,
) -> crate::Result<Vec<crate::vector::VectorHit>> {
    if let Some(entry) = named_entry(db, name, selection, id_column)? {
        let requested_metric = match metric {
            ...
            crate::vector::Metric::SquaredEuclidean => {
                crate::persistent::RetrievalMetric::SquaredEuclidean
            }
        };
        ...
        {
            return Err(PvError::Query(
                "Named vector index column/metric/dimensions definition mismatch".into(),
            ));
        }
        if selection.accelerate {
            ...
            let index = entry
                ...
                .vector
                .as_ref()
                .ok_or_else(|| crate::persistent::corrupt("missing admitted vector data"))?;
            return index
                .search_filtered(query, limit, &ids)
                .map_err(|error| PvError::Query(error.to_string()));
        }
    }
    vector_index(&selection.rows, id_column, column, query, metric)?
        .search(query, limit)
        .map_err(|error| PvError::Query(error.to_string()))
}
...
impl Database {
    ...
    pub(crate) fn retrieve_json_controlled(
        ...
        let request: RetrievalRequest =
            serde_json::from_str(request).map_err(|error| PvError::Query(error.to_string()))?;
        let hits = match request {
            ...
            RetrievalRequest::FullText {
                ...
                .into_iter()
                .map(|hit| serde_json::json!({"id": hit.id.to_string(), "score": hit.score}))
                .collect::<Vec<_>>()
            }
            #[cfg(feature = "vector-search")]
            RetrievalRequest::Vector {
                ...
                .into_iter()
                .map(|hit| serde_json::json!({"id": hit.id.to_string(), "distance": hit.distance}))
                .collect::<Vec<_>>()
            }
        };
        control.check()?;
...
fn vector_index(
    ...
    let vector = columns
        .iter()
        .position(|column| column == vector_column)
    ...
    for row in rows {
        ...
        let values: Vec<f32> =
            serde_json::from_str(encoded).map_err(|error| PvError::Query(error.to_string()))?;
        index
