...
mod qualification {
    ...
    fn vector(id: usize, dimensions: usize) -> String {
        serde_json::to_string(&(0..dimensions).map(|dimension| ((id + dimension) % 11 + 1) as f32).collect::<Vec<_>>()).unwrap()
    }
    ...
    fn populate(db: &mut Database, count: usize, dimensions: usize) -> picovolt::Result<()> {
        ...
        db.transaction(|db| {
            for id in 0..count {
                let body = format!("verified snapshot database commit history {}", "bounded local retrieval tokenization vectors durable tenant ownership ".repeat(5));
                db.query_with("INSERT INTO docs VALUES (?,?,?,?,?)", &[
                    Value::Int(id as i64), Value::Text(format!("tenant-{}", id % 8)),
                    Value::Text(format!("document {id} verified")), Value::Text(body), Value::Text(vector(id, dimensions)),
                ])?;
            }
            Ok(())
        })
    ...
    fn request(kind: &str, filtered: bool, dimensions: usize) -> Json {
        let columns = match kind { "full_text" => "id,title,body", "vector" => "id,embedding", _ => "*" };
        let sql = format!("SELECT {columns} FROM docs{}", if filtered { " WHERE tenant='tenant-0'" } else { "" });
        let query = vec![1.0f32; dimensions];
        match kind {
            "full_text" => json!({"kind":kind,"sql":sql,"id_column":"id","text_columns":["title","body"],"query":"verified snapshot","limit":10}),
            "vector" => json!({"kind":kind,"sql":sql,"id_column":"id","vector_column":"embedding","metric":"cosine","query":query,"limit":10}),
            _ => json!({"kind":"hybrid","sql":sql,"id_column":"id","text_columns":["title","body"],"vector_column":"embedding","query":"verified snapshot","vector_query":query,"metric":"cosine","text_weight":0.5,"candidate_limit":30,"limit":10}),
        }
    }
    ...
    fn time_queries(db: &mut Database, request: &str, repetitions: usize) -> picovolt::Result<f64> {
        let start = Instant::now();
        for _ in 0..repetitions { std::hint::black_box(db.retrieve_json(request)?); }
    ...
    fn directory_bytes(path: &Path) -> std::io::Result<u64> {
        ...
        for entry in std::fs::read_dir(path)? {
            let path = entry?.path();
            total += if path.is_dir() { directory_bytes(&path)? } else { std::fs::metadata(path)?.len() };
        }
        Ok(total)
    }
    ...
    fn rss() -> Json {
        ...
        let number = |key: &str| status.lines().find(|line| line.starts_with(key))
    ...
    fn corpus(count: usize, dimensions: usize) -> Result<Json, Box<dyn std::error::Error>> {
        let mut plain = Database::open_memory(); populate(&mut plain, count, dimensions)?;
        ...
        let start = Instant::now(); let reference = Database::import_bytes(&plain_bytes)?;
        let plain_reopen = start.elapsed().as_secs_f64() * 1000.0; drop(reference);
        let repetitions = 15;
        let mut queries = Vec::new();
        for kind in ["full_text", "vector", "hybrid"] {
            for filtered in [false, true] {
                let request = request(kind, filtered, dimensions);
                let legacy = request.to_string(); let named = named(request).to_string();
                assert_eq!(reopened.retrieve_json(&legacy)?, reopened.retrieve_json(&named)?, "differential failure before measuring");
                ...
                let (ephemeral, persistent) = if filtered {
                    ...
                    (time_queries(&mut reopened, &legacy, repetitions)?, persistent)
                } else {
        ...
        let temp = tempfile::tempdir()?; let root = temp.path().join("workspace");
        let mut workspace = Database::open_dev(&root)?; populate(&mut workspace, count, dimensions)?;
        let workspace_plain = directory_bytes(&root)?;
        indexes(&mut workspace, dimensions)?; let workspace_indexed = directory_bytes(&root)?;
        workspace.query("UPDATE docs SET body='verified snapshot changed' WHERE id=0")?;
        let workspace_after_update = directory_bytes(&root)?;
        drop(workspace);
        let start = Instant::now(); let workspace = Database::open_dev(&root)?;
        let workspace_reopen = start.elapsed().as_secs_f64()*1000.0; drop(workspace);
        Ok(json!({"documents":count,"vector_dimensions":dimensions,"text_create_ms":text_build,"vector_create_ms":vector_build,
    ...
    pub fn run() -> Result<(), Box<dyn std::error::Error>> {
        match std::env::args().nth(1).as_deref() {
            Some("--fixture") => {
                ...
                if path.exists() { return Err("Refusing to overwrite an existing golden fixture".into()); }
                let mut db = Database::open_memory(); populate(&mut db, 3, 3)?; indexes(&mut db, 3)?;
                let bytes = db.bake_to_bytes()?;
                let mut reopened = Database::import_bytes(&bytes)?;
                for kind in ["full_text", "vector", "hybrid"] {
                    let legacy = request(kind, false, 3);
                    assert_eq!(reopened.retrieve_json(&legacy.to_string())?, reopened.retrieve_json(&named(legacy).to_string())?);
                }
                let mut file = std::fs::OpenOptions::new().write(true).create_new(true).open(path)?;
                file.write_all(&bytes)?; file.sync_all()?;
                println!("Created {} ({} bytes, BLAKE3 {})", path.display(), bytes.len(), blake3::hash(&bytes));
            }
            Some("--bench") => {
                let corpora = vec![corpus(1_000, 128)?, corpus(10_000, 128)?];
                println!("{}", serde_json::to_string_pretty(&json!({"schema_version":1,"mandate":"PV-2.3-M-001",
