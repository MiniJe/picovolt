...
/* Library semantic version. Static; never NULL; do not free. */
...
/*
 * Validate and retain a reusable SQL template. The statement does not own db
 * and may be executed against another compatible PicoVolt handle.
 */
PvStmt *pv_prepare(const PvDb *db, const char *sql);

/* Number of positional `?` parameters required by a prepared statement. */
size_t pv_stmt_parameter_count(const PvStmt *stmt);

/* Execute a prepared statement with a JSON parameter array. The returned JSON
 * string must be freed with pv_string_free(). */
char *pv_stmt_execute(const PvStmt *stmt, PvDb *db, const char *params_json);

/* Free a prepared statement. NULL is ignored. */
void pv_stmt_close(PvStmt *stmt);
