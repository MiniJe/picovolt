...
const directory=resolve(process.env.PICOVOLT_JS_PACKAGE_DIR||'artifacts/npm-2.2.0');
test('2.2 WASM and JS adapter execute full-text, vector and hybrid retrieval',async()=>{
  ...
  const hybrid=db.retrieve({...text,kind:'hybrid',sql:'SELECT id,body,embedding FROM docs',vector_column:'embedding',vector_query:[1,0],metric:'cosine',text_weight:0.5,candidate_limit:10});
  assert.equal(hybrid[0].id,'1');assert.equal(hybrid[0].text_rank,1);assert.equal(hybrid[0].vector_rank,1);
