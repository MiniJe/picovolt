...
fn metadata(image: &[u8]) -> (usize, usize, Json) {
    ...
    (
        cas,
        manifest,
        serde_json::from_slice(&image[manifest..]).unwrap(),
    )
}

fn blob(image: &[u8], name: &str) -> Vec<u8> {
    let (cas, _, metadata) = metadata(image);
    let descriptor = metadata["retrieval_indexes"]
        .as_array()
        .unwrap()
        .iter()
        .find(|value| value["definition"]["name"] == name)
        .unwrap();
...
fn replace_blob(image: &[u8], name: &str, replacement: &[u8]) -> Vec<u8> {
    ...
    for (id, extent) in original_directory.iter().enumerate() {
        ...
        let bytes = if id == target {
            replacement
        } else {
            &image[cas + offset..cas + offset + length]
        };
    ...
    let mut output = image[..cas].to_vec();
    output.extend(pool);
...
fn rewrite_manifest(image: &[u8], update: impl FnOnce(&mut Json)) -> Vec<u8> {
    let (_, offset, mut metadata) = metadata(image);
    update(&mut metadata);
    let mut result = image[..offset].to_vec();
    result.extend(serde_json::to_vec(&metadata).unwrap());
    result
}
...
fn rejects(bytes: &[u8]) {
    ...
    assert!(
        ...
        "malformed retrieval index was accepted"
    );
}
...
fn envelope_magic_versions_lengths_checksum_and_trailing_bytes_are_checked() {
    let image = image();
    for name in ["ft", "vx"] {
        let original = blob(&image, name);
        validate_retrieval_index_bytes(&original).unwrap();
        for offset in [0, 8, 10, 11, 20, 21, 22, 23, original.len() - 1] {
            ...
            assert!(
                validate_retrieval_index_bytes(&changed).is_err(),
                "{name} at {offset}"
            );
...
fn canonical_rehash_cannot_make_wrong_document_ids_or_vectors_healthy() {
    let image = image();
    let mut vector = blob(&image, "vx");
    let start = body_start(&vector);
    ...
    vector[start + 20..start + 24].copy_from_slice(&0.5f32.to_le_bytes());
    rehash(&mut vector);
    validate_retrieval_index_bytes(&vector).unwrap();
    rejects(&replace_blob(&image, "vx", &vector));
    let mut vector = blob(&image, "vx");
    let start = body_start(&vector);
    vector[start + 12..start + 20].copy_from_slice(&(-7i64).to_le_bytes());
    rehash(&mut vector);
    validate_retrieval_index_bytes(&vector).unwrap();
    rejects(&replace_blob(&image, "vx", &vector));
    let mut text = blob(&image, "ft");
    let start = body_start(&text);
    text[start + 16..start + 24].copy_from_slice(&(-7i64).to_le_bytes());
    rehash(&mut text);
...
fn dimensions_counts_postings_and_descriptor_disagreement_fail_closed() {
    ...
    for name in ["ft", "vx"] {
        let original = blob(&image, name);
        let start = body_start(&original);
        for relative in [0, 4] {
            let mut changed = original.clone();
            changed[start + relative..start + relative + 4]
                .copy_from_slice(&u32::MAX.to_le_bytes());
            rehash(&mut changed);
            rejects(&replace_blob(&image, name, &changed));
        }
    }
    let original = blob(&image, "ft");
    let start = body_start(&original);
    for relative in [24, 28, 32, 36] {
        let mut changed = original.clone();
        changed[start + relative..start + relative + 4].copy_from_slice(&u32::MAX.to_le_bytes());
        rehash(&mut changed);
        rejects(&replace_blob(&image, "ft", &changed));
    }
    for field in ["encoded_bytes", "document_count", "generation", "cas_id"] {
        rejects(&rewrite_manifest(&image, |metadata| {
            metadata["retrieval_indexes"][0][field] = json!(u64::MAX)
        }));
    }
    rejects(&rewrite_manifest(&image, |metadata| {
        metadata["retrieval_indexes"][0]["definition"]["table"] = json!("missing")
    }));
    rejects(&rewrite_manifest(&image, |metadata| {
        metadata["retrieval_indexes"][0]["definition"]["id_column"] = json!("body")
    }));
    rejects(&rewrite_manifest(&image, |metadata| {
        metadata["retrieval_indexes"][0]["unexpected"] = json!(true)
    }));
    rejects(&rewrite_manifest(&image, |metadata| {
        metadata["retrieval_indexes"][1]["definition"]["name"] = json!("ft")
    }));
    let mut understated =
        rewrite_manifest(&image, |metadata| metadata["format_version"] = json!(7));
    understated[4..6].copy_from_slice(&7u16.to_le_bytes());
    rejects(&understated);
}
...
fn workspace_index_damage_fails_open_without_changing_source_files() {
    ...
    db.query("INSERT INTO docs VALUES (1,'authoritative')")
        .unwrap();
    db.query("CREATE INDEX ft ON docs USING FULLTEXT (body) WITH (id_column='id')")
        .unwrap();
    drop(db);
    let manifest: Json =
        serde_json::from_slice(&std::fs::read(root.join(picovolt::MANIFEST_FILE)).unwrap())
            .unwrap();
    let id = manifest["retrieval_indexes"][0]["cas_id"].as_u64().unwrap() as usize;
    let hash = manifest["cas_hashes"][id].as_str().unwrap();
    ...
    let mut db = Database::open_dev(&root).unwrap();
    assert_eq!(
        db.query("SELECT body FROM docs").unwrap().rows().unwrap()[0][0],
        picovolt::Value::Text("authoritative".into())
    );
}
