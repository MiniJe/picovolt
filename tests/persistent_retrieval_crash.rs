...
fn crash_child() {
    ...
    db.query(&format!(
        "INSERT INTO docs VALUES ({cycle},'commit candidate','[1,1]')"
    ))
    .unwrap();
    db.query(&format!(
        "UPDATE docs SET body='commit cycle {cycle}' WHERE id=0"
    ))
    .unwrap();
    db.query("UPDATE docs SET embedding='[0,1]' WHERE id=0")
        .unwrap();
    if commit {
    ...
    std::io::Write::write_all(&mut signal, b"ready").unwrap();
...
fn abrupt_exit_keeps_table_and_indexes_on_one_committed_state() {
    ...
    for logged in [false, true] {
        ...
        db.query("INSERT INTO docs VALUES (0,'commit baseline','[1,0]')")
            .unwrap();
        if logged {
            db.enable_commit_log(picovolt::CommitLogOptions::default())
                .unwrap();
        }
        ...
        for cycle in 1..=12 {
            ...
            let output = Command::new(std::env::current_exe().unwrap())
                ...
                .env(CHILD, &root)
                .env("PV23_CRASH_ID", cycle.to_string())
                .env("PV23_CRASH_COMMIT", if commit { "yes" } else { "no" })
                .env("PV23_CRASH_READY", &ready)
                .output()
                .unwrap();
            assert!(
                ready.is_file(),
            ...
            if commit {
                rows += 1;
                expected = db.verification_hash().unwrap();
            } else {
                assert_eq!(
                    db.verification_hash().unwrap(),
                    expected,
                    "uncommitted cycle {cycle} leaked"
                );
            }
            assert_eq!(db.row_count("docs", None).unwrap(), rows);
            assert_eq!(
                db.row_count("docs", None).unwrap() as usize,
                db.retrieval_indexes()[0].document_count
            );
            check(&mut db);
            drop(db);
        }
...
fn dropping_an_explicit_writer_rolls_back_retrieval_catalog_and_rows() {
    ...
    let mut db = Database::open_dev(&root).unwrap();
    db.query("CREATE TABLE docs (id,body,embedding)").unwrap();
    db.query("INSERT INTO docs VALUES (1,'commit baseline','[1,0]')")
        .unwrap();
    db.query("CREATE INDEX ft ON docs USING FULLTEXT (body) WITH (id_column='id')")
        .unwrap();
    db.query("CREATE INDEX vx ON docs USING VECTOR (embedding) WITH (id_column='id',metric='cosine',dimensions=2)").unwrap();
