...
fn populate(db: &mut Database) {
    db.query("CREATE TABLE docs (id,tenant,title,body,embedding,other)").unwrap();
    for (id, tenant, title, body, vector) in [
        (1, "a", "apple apple banana", Some("verified snapshot"), "[1,0,0]"),
        (2, "a", "apple banana banana", None, "[0,1,0]"),
        (3, "b", "Unicode Știință 東京", Some("snapshot database"), "[0,0,1]"),
    ] {
        db.query_with("INSERT INTO docs VALUES (?,?,?,?,?,0)", &[
            Value::Int(id), Value::Text(tenant.into()), Value::Text(title.into()),
            body.map_or(Value::Null, |value| Value::Text(value.into())), Value::Text(vector.into()),
        ]).unwrap();
    }
}
...
fn ddl_feature_gates_and_legacy_secondary_indexes() {
    ...
    assert_eq!(db.query("SELECT id FROM docs WHERE tenant='a'").unwrap().rows().unwrap().len(), 2);
}
...
fn named_catalog_conflicts_schema_validation_and_drop() {
    ...
    db.query(FT).unwrap();
    assert!(db.query(FT).is_err());
    db.query(&FT.replace("CREATE INDEX ft", "CREATE INDEX IF NOT EXISTS ft")).unwrap();
    assert!(db.query("CREATE INDEX IF NOT EXISTS ft ON docs USING FULLTEXT (title) WITH (id_column='id')").is_err());
...
mod both {
    ...
    fn named(mut request: Json) -> Json {
        match request["kind"].as_str().unwrap() {
            ...
            "vector" => request["index"] = if request["metric"] == "cosine" { json!("vx") } else { json!("l2") },
            "hybrid" => { request["text_index"] = json!("ft"); request["vector_index"] = json!("vx"); },
            _ => unreachable!(),
        }
        request
    }
    ...
    fn initial_build_and_all_storage_lifecycles() {
        let mut db = database(); indexed(&mut db);
        ...
        for logged in [false, true] {
            ...
            let mut dev = Database::open_dev(&root).unwrap(); populate(&mut dev);
            if logged { dev.enable_commit_log(picovolt::CommitLogOptions::default()).unwrap(); }
            indexed(&mut dev);
            ...
            for info in reopened.retrieval_indexes() {
                assert_eq!(info.health, "healthy"); assert!(!info.pending_write);
                assert!(info.persisted_bytes > 0);
                reopened.verify_retrieval_index(&info.definition.name).unwrap();
            }
        }
    }
    ...
    fn insert_update_id_delete_and_unindexed_updates_stay_exact() {
        ...
        db.query("INSERT INTO docs VALUES (4,'a','apple banana','new','[1,1,0]',0)").unwrap();
        db.query("UPDATE docs SET title='banana apple apple apple' WHERE id=2").unwrap();
        db.query("UPDATE docs SET embedding='[1,0.25,0]' WHERE id=2").unwrap();
        db.query("UPDATE docs SET id=9223372036854775807 WHERE id=4").unwrap();
        db.query("DELETE FROM docs WHERE id=3").unwrap();
        equivalent(&mut db, "SELECT * FROM docs WHERE tenant='a'");
        for info in db.retrieval_indexes() { db.verify_retrieval_index(&info.definition.name).unwrap(); }
    ...
    fn rollback_failed_statements_and_explicit_rebuild_preserve_state() {
        ...
        db.query("UPDATE docs SET title='different' WHERE id=2").unwrap();
        ...
        assert!(aborted.is_err()); assert_eq!(db.verification_hash().unwrap(), before);
        db.rebuild_retrieval_index("ft").unwrap();
        assert_eq!(db.retrieval_indexes().iter().find(|info| info.definition.name == "ft").unwrap().rebuild_count, 1);
        db.verify_retrieval_index("ft").unwrap();
        db.query("BEGIN").unwrap(); db.query("DROP INDEX ft").unwrap(); db.query("ROLLBACK").unwrap();
        equivalent(&mut db, "SELECT * FROM docs");
    }
    ...
    fn multi_statement_and_low_level_mutations_share_the_commit_boundary() {
        ...
        db.transaction(|database| {
            database.insert("docs", vec![Value::Int(5),Value::Text("a".into()),Value::Text("apple banana".into()),Value::Null,Value::Text("[1,1,1]".into()),Value::Int(0)])?;
            database.update("docs", "title", &Value::Text("apple".into()), "id", &Value::Int(1))?;
            database.delete("docs", "id", &Value::Int(3))?;
            Ok(())
        }).unwrap();
        equivalent(&mut db, "SELECT * FROM docs");
        db.query("DROP TABLE docs").unwrap(); assert!(db.retrieval_indexes().is_empty());
    }

    #[test]
    fn tenant_bm25_is_not_global_bm25_with_post_filtering() {
        let mut db = Database::open_memory();
        db.query("CREATE TABLE docs (id,tenant,title,body,embedding)").unwrap();
        db.query("INSERT INTO docs VALUES (1,'a','apple apple banana','','[1,0,0]')").unwrap();
        db.query("INSERT INTO docs VALUES (2,'a','apple banana banana','','[1,0,0]')").unwrap();
        for id in 3..33 { db.query(&format!("INSERT INTO docs VALUES ({id},'b','apple','','[0,1,0]')")).unwrap(); }
        indexed(&mut db);
        let global = requests("SELECT * FROM docs").remove(0);
        let tenant = requests("SELECT * FROM docs WHERE tenant='a'").remove(0);
        let global: Json = serde_json::from_str(&db.retrieve_json(&global.to_string()).unwrap()).unwrap();
        let filtered: Json = serde_json::from_str(&db.retrieve_json(&tenant.to_string()).unwrap()).unwrap();
        assert_eq!(global[0]["id"], "2"); assert_eq!(filtered[0]["id"], "1");
        equivalent(&mut db, "SELECT * FROM docs WHERE tenant='a'");
    ...
    fn historical_queries_fall_back_and_definition_mismatches_fail() {
        ...
        db.query("UPDATE docs SET title='no longer matching' WHERE id=1").unwrap();
        db.query("UPDATE docs SET embedding='[-1,0,0]' WHERE id=2").unwrap();
        db.query("DELETE FROM docs WHERE id=3").unwrap();
        equivalent(&mut db, &format!("SELECT * FROM docs BEFORE {previous}"));
        equivalent(&mut db, "SELECT * FROM docs WHERE tenant='a' ORDER BY id DESC LIMIT 1");
        let mut request = named(requests("SELECT * FROM docs").remove(0));
        for (field, value) in [("index",json!("missing")),("id_column",json!("other")),("text_columns",json!(["title"]))] {
            let old = request[field].clone(); request[field] = value;
            assert!(db.retrieve_json(&request.to_string()).is_err()); request[field] = old;
        }
    ...
    fn generated_corpora_preserve_unicode_nulls_ties_and_both_vector_metrics() {
        ...
        for id in 4..124 {
            let title = match id % 6 {
                0 => "apple apple banana".into(), 1 => "APPLE banana".into(),
                2 => "Știință 東京 apple".into(), 3 => String::new(),
                4 => format!("{} banana apple", "z".repeat(129)), _ => "banana banana apple".into(),
            };
            let vector = format!("[{}, {}, {}]", id % 7 + 1, id % 5, -(id % 3));
            db.query_with("INSERT INTO docs VALUES (?,?,?,?,?,0)", &[Value::Int(id),Value::Text(if id % 2 == 0 { "a" } else { "b" }.into()),Value::Text(title),Value::Null,Value::Text(vector)]).unwrap();
        }
        indexed(&mut db);
        for filter in ["", " WHERE tenant='a'", " WHERE tenant='b'", " WHERE id>90", " WHERE id<0"] {
    ...
    fn cli_retrieve_and_inspect_expose_persisted_indexes() {
        let temp = tempfile::tempdir().unwrap(); let path = temp.path().join("docs.pvdb");
        let request_path = temp.path().join("request.json");
        let mut db = database(); indexed(&mut db); db.bake(&path).unwrap();
        ...
        let output = std::process::Command::new(env!("CARGO_BIN_EXE_pv")).arg("retrieve").arg(&path).arg(request_path).output().unwrap();
